...
$sstart = strtotime('2019-03-16 00:00:00');
...
if ($datafound > 10) {
	...
	for ($page = 0; $page <= $totalpages; $page++) {
		...
		foreach ($response->hits as $data) {
			...
			$sourceexist = "SELECT id_source FROM knewin_radio WHERE id_source = ".$didsource;
			$resultsource = $dbcon->query($sourceexist);
			if ($resultsource->num_rows == 0) {
				$insertsource = "INSERT INTO knewin_radio (id_source, source) VALUES (".$didsource.", '".utf8_decode($dsource)."')";
